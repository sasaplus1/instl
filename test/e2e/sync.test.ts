  ...
  describe("touch operation", () => {
    ...
    it("should set mode for new file", () => {
      const recipe = createTestFile(
        tempDir,
        "recipe.ltsv",
        `op:touch\tdest:${tempDir}/newfile.txt\tmode:0600`,
      );
    ...
    it("should create parent directories", () => {
      const recipe = createTestFile(
        tempDir,
        "recipe.ltsv",
        `op:touch\tdest:${tempDir}/a/b/c/file.txt`,
      );

      runInstl(`sync ${recipe}`);
      assert.strictEqual(
        fileExists(path.join(tempDir, "a/b/c/file.txt")),
        true,
      );
    });
  });
  ...
  describe("mkdir operation", () => {
    ...
    it("should set directory mode", () => {
      const recipe = createTestFile(
        tempDir,
        "recipe.ltsv",
        `op:mkdir\tdest:${tempDir}/newdir\tmode:0700`,
      );
  ...
  describe("cp operation", () => {
    it("should copy file", () => {
      ...
      const recipe = createTestFile(
        tempDir,
        "recipe.ltsv",
        `op:cp\tsrc:${src}\tdest:${tempDir}/dest.txt`,
      );
    ...
    it("should set file mode", () => {
      ...
      const recipe = createTestFile(
        tempDir,
        "recipe.ltsv",
        `op:cp\tsrc:${src}\tdest:${tempDir}/dest.txt\tmode:0600`,
      );
    ...
    it("should resolve relative paths from recipe directory", () => {
      ...
      const recipe = createTestFile(
        tempDir,
        "recipes/recipe.ltsv",
        `op:cp\tsrc:../src/source.txt\tdest:${tempDir}/dest.txt`,
      );
  ...
  describe("ln operation", () => {
    it("should create symlink", () => {
      ...
      const recipe = createTestFile(
        tempDir,
        "recipe.ltsv",
        `op:ln\tsrc:${src}\tdest:${tempDir}/link`,
      );
  ...
  describe("environment variable expansion", () => {
    it("should expand $HOME in dest", () => {
      ...
      const recipe = createTestFile(
        tempDir,
        "recipe.ltsv",
        `op:touch\tdest:$INSTL_TEST_DIR/expanded.txt`,
      );
...
`,
      ...
      assert.strictEqual(
        readFile(path.join(tempDir, "config/app.conf")),
        "content",
      );
      assert.strictEqual(
        getFileMode(path.join(tempDir, "config/app.conf")),
        0o644,
      );

      assert.strictEqual(
        isSymlink(path.join(tempDir, "data/app.conf.link")),
        true,
      );
